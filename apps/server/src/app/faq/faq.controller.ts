...
export class FaqController {
  ...
  async index(
  ...
  ): Promise<ApiPaginateResponse<Faq>> {
    let query;
    if (params.keyword) {
      query = await this.faqService.repository
        .createQueryBuilder()
    ...
    const result = await this.faqService.paginate(query, {
      page: params.page ?? 1,
  ...
  @Get(':id')
  async show(
    @Param('id', ParseIntPipe) faqId: number
  ): Promise<ApiItemResponse<Faq>> {
  ...
  async create(@Body() data: CreateFaqDto): Promise<ApiItemResponse<Faq>> {
    const result = await this.faqService.create(
      pick(data, ['question', 'answer'])
    );
    return this.response.item(result, FaqTransformer);
  }
  ...
  async update(
    ...
    const result = await this.faqService.update(
      faqId,
      pick(data, ['question', 'answer'])
    );
    return this.response.item(result, FaqTransformer);
