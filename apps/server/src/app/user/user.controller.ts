...
export class UserController {
  ...
  @Get('/list')
  @Auth('admin')
  @ApiOperation({ summary: 'Danh sách người dùng bao gồm cả admin' })
  async indexs(
    @Query() params: FindManyQueryParam
  ): Promise<ApiPaginateResponse<User>> {
    const page =
    ...
    const limit =
      params.limit && Number(params.limit) > 0
        ...
        : 20;
    let query = this.userService.repository
    ...
    if (params.keyword) {
      ...
      query = query.andWhere(
        new Brackets((sqb) => {
          sqb
            .where('user.email LIKE :searchString', { searchString })
  ...
  @Auth('admin')
  async createAccount(
    @Body() data: AdminCreateAccountDto
