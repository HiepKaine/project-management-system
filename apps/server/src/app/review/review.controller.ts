...
import { ApiItemResponse, ApiPaginateResponse, ApiResponseService, ApiSuccessResponse, Auth } from '@server/common';
...
export class ReviewController {
  ...
  @Get()
  @Auth('admin')
  async index(@Param() param: FindManyQueryParam): Promise<ApiPaginateResponse<Review>> {
    ...
    if (param.keyword) {
      query = query.andWhere('review.review LIKE :keyword', { keyword: `%${param.keyword}%` })
    }
  ...
  @Get(':id')
  async show(@Param('id', ParseIntPipe) id: number): Promise<ApiItemResponse<Review>> {
  ...
  @Post()
  @Auth('admin')
  async create(@Body() data: CreateReviewDto): Promise<ApiItemResponse<Review>> {
  ...
  @Put(':id')
  @Auth('admin')
  async update(@Param('id', ParseIntPipe) id: number, @Body() data: UpdateReviewDto): Promise<ApiItemResponse<Review>> {
    const result = await this.reviewService.update(id, pick(data, ['image', 'user', 'review', 'rateCount']));
    return this.response.item(result, ReviewTransformer)
  }

  @Delete(':id')
  @Auth('admin')
  async delete(@Param('id', ParseIntPipe) id: number): Promise<ApiSuccessResponse> {
