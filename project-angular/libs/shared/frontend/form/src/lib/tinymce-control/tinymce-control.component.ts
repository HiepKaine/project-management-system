...
import { ApiCollectionResponse } from 'libs/shared/frontend/common/src/index';
...
export class TinymceControlComponent
  extends NgxFormManagerComponent<TinyMceControlOption>
  implements OnInit
{
  ...
  constructor(
    ...
    @Inject(TinymceControlConfigService) public config: TinymceControlConfig
  ) {
    super();
    this.config = this.config ?? {
      ...
      toolbar:
        'undo redo | bold italic underline | alignleft aligncenter alignright alignjustify | outdent indent | link image | table ',
      image_title: true,
      height: '300',
      ...
      file_picker_callback: (callback: any, value: any, meta: any) => {
        if (meta.filetype == 'image') {
          ...
          el.onchange = (e: any) => {
            ...
            formData.append(
              this.option.queryParamKey || 'files',
              e.target.files[0]
            );
            this.http
              .post<ApiCollectionResponse<{ url: string }>>(
                this.option.apiEndPoint,
                formData
              )
              .pipe(
                catchError((error) => {
